...
namespace GXPEngine
{
    ...
    public class Canvas : Sprite
    {
        ...
        /// </param>
        public Canvas(int width, int height, bool addCollider = true, uint CollisionLayers = 0xFFFFFFFF, uint CoupleWithLayers = 0xFFFFFFFF) : this(new Bitmap(width, height), addCollider, CollisionLayers, CoupleWithLayers)
        {
            name = width + "x" + height;
        }

        public Canvas(System.Drawing.Bitmap bitmap, bool addCollider = true, uint CollisionLayers = 0xFFFFFFFF, uint CoupleWithLayers = 0xFFFFFFFF) : base(bitmap, addCollider, CollisionLayers, CoupleWithLayers)
        {
            _graphics = Graphics.FromImage(bitmap);
            _invalidate = true;
        }

        public Canvas(string filename, bool addCollider = true, uint CollisionLayers = 0xFFFFFFFF, uint CoupleWithLayers = 0xFFFFFFFF) : base(filename, false, addCollider, CollisionLayers, CoupleWithLayers)
        {
